...
gui:
	gcc -Wall -g -o milsgui utils.c milsgui.c log.c cmdloopgui.c -L/usr/lib/x86_64-linux-gnu -lssl -lcrypto `pkg-config --cflags --libs gtk+-3.0` -export-dynamic
install:
...
clean:
	rm -rf mils milsd milsgui

addtestuser:
	useradd test
	passwd test
	mkdir -p /home/mils/test
